...
contract ExchangeIssuanceV2 is ReentrancyGuard {
     ...
     */
    function approveSetToken(ISetToken _setToken)
        external
    ...
    function issueSetForExactToken(
    ...
    ) external isSetToken(_setToken) nonReentrant returns (uint256) {
        ...
        uint256 setTokenAmount = _issueSetForExactWETH(
            _setToken,
            _minSetReceive,
            amountEth
        );
    ...
    function getEstimatedIssueSetAmount(
        ...
        IERC20 _inputToken,
        uint256 _amountInput
    ) external view isSetToken(_setToken) returns (uint256) {
        require(_amountInput > 0, "ExchangeIssuance: INVALID INPUTS");
     ...
     */
    function _issueExactSetFromWETH(
        ISetToken _setToken,
        uint256 _amountSetToken,
        uint256 _maxEther
    ) internal returns (uint256) {
        ...
        require(
            sumEth <= _maxEther,
            "ExchangeIssuance: INSUFFICIENT_INPUT_AMOUNT"
        );
     ...
     */
    function _redeemExactSet(ISetToken _setToken, uint256 _amount)
        internal
        returns (uint256)
    {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _components.length; i++) {
            // Check that the component does not have external positions
            require(
                _setToken.getExternalPositionModules(_components[i]).length ==
                    0,
                "ExchangeIssuance: EXTERNAL_POSITIONS_NOT_ALLOWED"
            );

            // Get minimum amount of ETH to be spent to acquire the required amount of SetToken component
            uint256 unit = uint256(
                _setToken.getDefaultPositionRealUnit(_components[i])
            );
        ...
        for (uint256 i = 0; i < _components.length; i++) {
            ...
            if (exchanges[i] == Exchange.None) {
                ...
                (uint256 reserveIn, uint256 reserveOut) = UniSushiV2Library
                    .getReserves(pairAddresses[i], WETH, _components[i]);
     ...
     */
    function _swapTokenForWETH(IERC20 _token, uint256 _amount)
        internal
        returns (uint256)
    {
        ...
        return
            _swapExactTokensForTokens(exchange, address(_token), WETH, _amount);
    }
